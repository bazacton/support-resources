...
import {validateArticleDatatableSearch} from '@app/help-center/articles/article-datatable/validate-article-datatable-search';
...
export function Component() {
  const {aiAgentId} = useParams();
  const {columns, filters} = useMemo(() => {
    const columns = ArticleDatatableColumns.filter(
      c => c.key !== 'ai_agents_count' || !!aiAgentId,
    );
    const filters = ArticleDatatableFilters.filter(
      f => f.key !== 'ai_agents_count' || !!aiAgentId,
    );
  ...
  const {
    ...
    setSearchQuery,
    isFiltering,
  } = useDatatableSearchParams(validateArticleDatatableSearch);

  const query = useDatatableQuery(
    helpCenterQueries.articles.index(searchParams, aiAgentId),
  );
